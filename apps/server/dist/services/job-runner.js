...
async function processPageLlm(job) {
    const rows = await db.execute(sql `SELECT * FROM pages WHERE id = ${job.entity_id} LIMIT 1`);
...
export async function runOneJob(workerId = `worker-${process.pid}`) {
  ...
  `);
    ...
    if (!job)
        return false;
    try {
        if (job.type === 'page.process_llm')
            await processPageLlm(job);
