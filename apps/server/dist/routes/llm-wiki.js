...
llmWikiRoutes.get('/inbox', async (c) => {
    ...
    if (!(await canViewSpace(user.id, spaceId)))
...
llmWikiRoutes.post('/topics/:topicId/accept', async (c) => {
    ...
    if (!(await canEditSpace(user.id, topic.spaceId)))
        return c.json({ error: 'Forbidden' }, 403);
...
llmWikiRoutes.post('/suggestions/bulk-accept', async (c) => {
    ...
    if (!(await canEditSpace(user.id, body.spaceId)))
        return c.json({ error: 'Forbidden' }, 403);
...
llmWikiRoutes.get('/graph', async (c) => {
    ...
    if (!workspaceId || !spaceId || !sourceId)
        return c.json({ error: 'workspaceId, spaceId, sourceId are required' }, 400);
    if (!(await canViewSpace(user.id, spaceId)))
        return c.json({ error: 'Forbidden' }, 403);
