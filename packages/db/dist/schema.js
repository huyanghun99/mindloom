...
import { boolean, index, integer, jsonb, pgEnum, pgTable, primaryKey, text, timestamp, uniqueIndex, uuid, vector } from 'drizzle-orm/pg-core';
...
export const pages = pgTable('pages', {
    ...
    contentJson: jsonb('content_json').$type().notNull().default({ type: 'doc', content: [] }),
...
export const ragSessions = pgTable('rag_sessions', {
    ...
    citations: jsonb('citations').$type().notNull().default([]),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow()
}, (t) => ({
...
export const shares = pgTable('shares', {
    ...
    snapshotContentJson: jsonb('snapshot_content_json').$type(),
